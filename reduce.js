...
function longestName(persons){
	var currentName=persons[0].name.first.length + persons[0].name.last.length;
var long= persons.reduce((acc,singleName)=>{
 
if ( singleName.name.first.length + singleName.name.last.length > currentName){
	 
}

return singleName.name.first + singleName.name.last ;
},0)
console.log(long);
}
...
*/
function  maxNumber(newArr){ 
var result= newArr.reduce((acc1 , max)=>{
